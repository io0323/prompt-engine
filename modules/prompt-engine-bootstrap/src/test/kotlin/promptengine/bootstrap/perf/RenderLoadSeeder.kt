 ...
 * （submitForReview/approveはADR-0016・Issue #9、[promptengine.bootstrap.PromptLifecycleSmokeTest]
 * と同じ制約。publish自体はエンドポイントがあるが、承認をHTTP側で作れない以上どのみち
 * ここでまとめて行う方が単純）ため、このJVM自身が`PromptRepository`（対象コンテナと
 ...
 *
 * 通常は`tools/perf/render_load_test.sh`から必要な環境変数（`PE_DATASOURCE_URL`・
 * `PERF_APP_BASE_URL`・`PERF_JWT`）を設定したうえで自動的に呼び出される（README「性能測定」節）。
 * 手動実行例:
 * ```
...
class RenderLoadSeeder {
    ...
    fun `対象コンテナへPublished状態のPromptVersionを1件用意する`() {
        ...
        val jwt = requireNotNull(System.getenv("PERF_JWT")) { "PERF_JWT環境変数が未設定です（DevJwks.javaの出力を参照）" }
        ...
        post(
            ...
            jwt,
            """{"key":"$promptKey","name":"Perf Load Prompt","semVer":"$semVerText",""" +
                """"source":${jsonString(PRODUCTION_SCALE_SOURCE)}}""",
        ).expectStatus(201, "create prompt")
    ...
    private fun approveAndPublishDirectly(
        ...
        val context =
            EventContext(actor = "perf-seed-tool", traceId = UUID.randomUUID().toString(), occurredAt = Instant.now())
    ...
    companion object {
         ...
         * few-shot例を複数含む。[tests/prompt-regression]のGolden回帰対象と同一ファイルを
         * 参照する（`tests/prompt-regression/fixtures/valid/03-production-scale-support-agent.prompt`）。
         */
