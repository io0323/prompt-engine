...
- 設計上の決定をコミットメッセージやコードコメントにのみ記録しない。設計書の記述と
  実装が食い違う場合、または観測可能な意味論が確定する場合は、ADRと設計書本体の
  両方に反映する。
...
5. 各フェーズの完了時に、新規追加コードの未カバー分岐を「本物のガード欠落 / 到達不能 /
   ツール由来」の3分類で監査し、結果を報告する。閾値を満たしていることは監査を省略する
   理由にならない。本物のガード欠落はテストを追加し、到達不能なコードは削除する。
6. フェーズ完了時の報告には、プロンプトで指定された各テスト要件に対して「それを満たす
   テストクラス・メソッド」を対応表として示す。「網羅した」という記述だけでは足りない。
   要件に対応するテストが存在しないものは、その旨を明示的に報告する。
   （理由: P9cで認可マトリクス（200/403/401）を要件に含めたが実装されず、5フェーズ後
   （Issue #115）まで誰も気づかなかったため。）
