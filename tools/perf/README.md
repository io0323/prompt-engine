...
| ケース | 内容 | 想定される課金 |
|---|---|---|
| コンテキスト長超過 | 固定の十分に大きい入力を送る（`oversizedContent()`、既知のどのモデルの上限も超える想定。ローカルの`ModelProfile`設定は読まない） | 最小（モデル実行前に拒否） |
| 無効なAPIキー | 意図的に不正な`Authorization`ヘッダで送る | なし（認証段階で拒否） |
| 存在しないモデル名 | 固定の存在しないモデル名文字列で送る（`NONEXISTENT_MODEL_NAME`） | なし（モデル解決段階で拒否） |
