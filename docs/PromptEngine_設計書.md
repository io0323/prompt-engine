...
| ID | 分類 | 要件 | 目標値 |
|---|---|---|---|
| NFR-001 | 可用性 | 24時間365日稼働、Read系はキャッシュで縮退継続（`PromptCache`実装はRedis到達不能時も`get`/`put`/`invalidateByPrompt`が例外を投げず、キャッシュミス相当として処理を継続する。ADR-0033追加決定e、`RedisPromptCacheDegradationIntegrationTest`で検証） | 99.9%（Read 99.99%） |
