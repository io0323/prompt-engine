...
作られたVersionは、実行記録の不在から参照ゼロを結論できないため**恒久的に`force=true`専用の
まま**残る。判断不能を許可側へ倒すと「参照されているものを誤って落とさない」というガードの
目的そのものを果たさなくなるため、拒否側へ倒す。将来これを解消するには、別の参照追跡手段
（AACP側のクライアント登録等）を導入するか、運用でカットオーバーを引き直す必要がある。
